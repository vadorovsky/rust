...
impl<'a> Parser<'a> {
    ...
    /// Built-in macro for BTF field-info expressions.
    pub(crate) fn parse_expr_btf_field_info(
        ...
        if let Err(mut e) = self.expect_one_of(&[], &[exp!(CloseParen)]) {
            if trailing_comma {
                e.note("unexpected third argument to BTF field-info macro");
            } else {
                e.note("BTF field-info macros expect dot-separated field names");
            }
            e.emit();
        }
        ...
        let span = lo.to(self.token.span);
        Ok(self.mk_expr(span, ExprKind::BtfFieldInfo(container, fields, kind)))
    }
