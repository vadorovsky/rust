...
pub enum Rvalue<'tcx> {
    ...
    /// Queries BTF-relocatable field metadata for a statically known field path.
    ///
    /// Backends with BTF relocation support may lower this to a relocation-aware
    /// field-info query. Other backends fall back to the field path's ordinary
    /// layout-computed result.
