...
impl<'a, 'll, 'tcx> BuilderMethods<'a, 'tcx> for Builder<'a, 'll, 'tcx> {
    ...
    fn btf_field_info(
        ...
        if self.cx.tcx.sess.target.arch != Arch::Bpf {
            self.cx.tcx.dcx().span_fatal(
                self.span,
                "BTF field relocation queries are only supported for BPF targets",
            );
        }
        if self.cx.dbg_cx.is_none() {
            self.cx
                .tcx
                .dcx()
                .span_fatal(self.span, "BTF field relocation queries require `-C debuginfo=2`");
        }
        ...
        for step in path {
            ...
            field_ptr = match step.container_ty.kind() {
                ty::Adt(adt, _) if adt.is_union() => self.btf_preserve_union_access_index(
                    step.container_ty,
                    field_ptr,
                    step.field.index() as u64,
                ),
                ty::Adt(..) | ty::Tuple(..) => {
                ...
                _ => bug!("unsupported BTF field-info container: {:?}", step.container_ty),
            };
        }
